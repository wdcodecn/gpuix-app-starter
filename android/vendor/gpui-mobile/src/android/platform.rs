...
struct AndroidPlatformState {
    ...
    system_wake_callback: Option<Box<dyn FnMut() + Send>>,
...
impl AndroidPlatform {
    ...
    pub fn new(headless: bool) -> Self {
        ...
        let dispatcher = if headless {
            ...
            let app = crate::android::jni::android_app()
                .expect("AndroidApp must be available before creating AndroidPlatform");
            AndroidDispatcher::new(&app)
        };
    ...
    pub fn quit(&self) {
        ...
        let should_quit = callback.as_mut().map(|cb| cb()).unwrap_or(true);
        if self.state.lock().quit_callback.is_none() {
            self.state.lock().quit_callback = callback;
        }
        if should_quit {
    ...
    pub fn did_become_active(&self) {
        ...
        let mut callback = self.state.lock().system_wake_callback.take();
        if let Some(callback) = callback.as_mut() {
            callback();
        }
        if self.state.lock().system_wake_callback.is_none() {
            self.state.lock().system_wake_callback = callback;
        }
    }
...
impl Platform for AndroidPlatform {
    ...
    fn on_quit(&self, callback: Box<dyn FnMut() -> bool>) {
        self.state.lock().quit_callback = Some(unsafe {
            std::mem::transmute::<Box<dyn FnMut() -> bool>, Box<dyn FnMut() -> bool + Send>>(callback)
        });
    }

    fn on_system_wake(&self, callback: Box<dyn FnMut()>) {
        self.state.lock().system_wake_callback = Some(unsafe {
