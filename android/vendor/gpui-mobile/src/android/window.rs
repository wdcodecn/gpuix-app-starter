...
// Keep Android at a low-power 30 Hz while the app is visible but idle. The
// renderer is demand-driven, so static screens do not repaint; an active touch
// or animation temporarily promotes to the user's selected target. After a
// longer timeout, `DEEP_IDLE_FRAME_RATE` releases the app's preference too.
const POWER_BALANCED_FRAME_RATE: f32 = 30.0;
const DEFAULT_INTERACTION_FRAME_RATE: f32 = 90.0;
/// A zero request releases the app's preference and lets Android choose the
/// panel's lowest/default mode while the app has been untouched for a while.
const DEEP_IDLE_FRAME_RATE: f32 = 0.0;

// Resolve an optional native-window symbol at runtime.
//
...
// always go through `dlopen`/`dlsym`. On Android these functions live in
// libnativewindow.so, which is not guaranteed to be visible through
// RTLD_DEFAULT when the app's other native libraries were loaded locally.
fn load_libandroid_symbol(name: &[u8]) -> *mut c_void {
    debug_assert_eq!(name.last().copied(), Some(0));
    static LIBNATIVEWINDOW: OnceLock<usize> = OnceLock::new();
    let handle = *LIBNATIVEWINDOW.get_or_init(|| unsafe {
        libc::dlopen(
            b"libnativewindow.so\0".as_ptr().cast(),
            libc::RTLD_NOW,
        ) as usize
    });
    if handle != 0 {
        unsafe { libc::dlsym(handle as *mut c_void, name.as_ptr().cast()) }
    } else {
        // A few vendor images expose the symbols globally even when the
        // library name is hidden; retain a safe fallback for those builds.
...
/// Request a power-balanced refresh rate from the native window when the
/// platform exposes the API.
///
...
fn set_native_frame_rate(window: &NativeWindow, frame_rate: f32) {
    ...
    match status {
        Some(0) => log::info!("Requested {:.0} Hz frame rate", frame_rate),
...
fn request_power_balanced_frame_rate(window: &NativeWindow) {
    set_native_frame_rate(window, POWER_BALANCED_FRAME_RATE);
}
...
struct WindowState {
    ...
    ime_insets: SafeAreaInsets,
    has_system_insets: bool,
    ...
    insets_callback: Option<Box<dyn FnMut(gpui::WindowInsets) + Send>>,
}
...
pub struct AndroidWindow {
    ...
    /// Set by GPUI invalidation/schedule_frame and consumed by the Android
    /// event loop. Keeping this demand-driven avoids repainting a static
    /// surface thousands of times per second while the app is idle.
    frame_requested: AtomicBool,
    /// Whether a touch scroll or momentum fling is active.
    interactive: AtomicBool,
    /// User-selected maximum for active touch/scroll/animation. Android may
    /// clamp this to a supported panel mode or a temporary system policy cap.
    power_frame_rate: AtomicU32,
    /// Highest interaction refresh rate allowed by the panel and system
    /// setting (for example 90/120/144/165 Hz).
    interaction_frame_rate: AtomicU32,
    /// Last refresh rate requested from Android, in Hz.
...
impl AndroidWindow {
    ...
    pub fn new(
        ...
        request_power_balanced_frame_rate(&native_window);
        ...
        let state = Arc::new(Mutex::new(WindowState {
            ...
            ime_insets: SafeAreaInsets::default(),
            has_system_insets: false,
        ...
        Ok(Arc::new(Self {
            ...
            frame_requested: AtomicBool::new(true),
            interactive: AtomicBool::new(false),
            power_frame_rate: AtomicU32::new(POWER_BALANCED_FRAME_RATE as u32),
            interaction_frame_rate: AtomicU32::new(
                crate::android::jni::query_interaction_frame_rate()
                    .unwrap_or(DEFAULT_INTERACTION_FRAME_RATE) as u32,
            ),
            requested_frame_rate: AtomicU32::new(POWER_BALANCED_FRAME_RATE as u32),
        }))
    }
    ...
    pub fn headless(width: i32, height: i32, scale_factor: f32) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            interactive: AtomicBool::new(false),
            power_frame_rate: AtomicU32::new(POWER_BALANCED_FRAME_RATE as u32),
            interaction_frame_rate: AtomicU32::new(DEFAULT_INTERACTION_FRAME_RATE as u32),
            requested_frame_rate: AtomicU32::new(POWER_BALANCED_FRAME_RATE as u32),
        })
    }
    ...
    pub fn init_window(&self, native_window: NativeWindow, gpu_context: GpuContext) -> Result<()> {
        request_power_balanced_frame_rate(&native_window);
        self.interaction_frame_rate.store(
            crate::android::jni::query_interaction_frame_rate()
                .unwrap_or(DEFAULT_INTERACTION_FRAME_RATE) as u32,
            Ordering::Release,
        );
        ...
        self.interactive.store(false, Ordering::Release);
        self.requested_frame_rate
            .store(POWER_BALANCED_FRAME_RATE as u32, Ordering::Release);
    ...
    /// Ask the Android event loop for one future frame. This is separate from
    /// `request_frame`, which consumes the request and runs the GPUI callback.
    pub fn schedule_frame(&self) {
        self.frame_requested.store(true, Ordering::Release);
        // GPUI invalidations can originate on the Hermes worker. Wake the
        // Android looper so a deeply-idle app reacts immediately instead of
        // waiting for its coarse idle poll timeout.
        if let Some(app) = crate::android::jni::android_app() {
            app.create_waker().wake();
        }
    }

    /// Consume one pending frame request.
    pub fn take_frame_request(&self) -> bool {
        self.frame_requested.swap(false, Ordering::AcqRel)
    }

    /// Check whether a frame is waiting without consuming the request.
    pub fn has_frame_request(&self) -> bool {
        self.frame_requested.load(Ordering::Acquire)
    }

    /// Whether high-refresh interaction pacing is active.
    pub fn is_interactive(&self) -> bool {
        self.interactive.load(Ordering::Acquire)
    }

    /// Switch between power-balanced idle refresh and high-refresh input
    /// pacing. The native-window call is issued only when the mode changes.
    pub fn set_interactive(&self, interactive: bool) {
        self.interactive.store(interactive, Ordering::Release);
        let frame_rate = if interactive {
            // Re-read the display policy when a gesture starts. Android may
            // have changed the effective cap since the last gesture (for
            // example after Battery Saver was enabled). We keep the user's
            // selected target intact and let SurfaceFlinger clamp the actual
            // panel rate to whatever the system currently permits.
            let detected = crate::android::jni::query_interaction_frame_rate()
                .unwrap_or_else(|| self.interaction_frame_rate.load(Ordering::Acquire) as f32);
            self.interaction_frame_rate
                .store(detected.round() as u32, Ordering::Release);
            let selected = self.power_frame_rate.load(Ordering::Acquire) as f32;
            selected.min(detected)
        } else {
            POWER_BALANCED_FRAME_RATE
        };
        self.request_frame_rate(frame_rate);
    }

    /// Set the user-selected active interaction ceiling from the app settings.
    /// Static and idle content keeps the fixed low-power budget below; this
    /// preference is applied only while touching, scrolling, animating, or
    /// otherwise actively repainting.
    /// Accepted common values are 20/30/40/50/60/90/120/144/165 Hz; the UI
    /// disables values absent from the current display's supported modes.
    pub fn set_power_frame_rate(&self, frame_rate: f32) {
        let rounded = frame_rate.round() as u32;
        if !matches!(rounded, 20 | 30 | 40 | 50 | 60 | 90 | 120 | 144 | 165) {
            log::warn!("ignoring unsupported power frame rate: {frame_rate}");
            return;
        }
        self.power_frame_rate.store(rounded, Ordering::Release);
        if self.is_interactive() {
            let cap = self.interaction_frame_rate.load(Ordering::Acquire) as f32;
            self.request_frame_rate((rounded as f32).min(cap));
        } else {
            // A preference change while idle must not promote a static page to
            // the selected high-refresh rate; keep the low-power idle budget.
            self.request_frame_rate(POWER_BALANCED_FRAME_RATE);
        }
    }

    /// Enter the deepest app-idle mode after a long period without input.
    /// This clears the app's refresh-rate preference; Android can then apply
    /// device-specific battery policy. The next touch promotes to the
    /// selected target, clamped by the current system policy.
    pub fn enter_deep_idle(&self) {
        if self.interactive.swap(false, Ordering::AcqRel) {
            log::info!("interaction pacing: deep idle requested while active");
        }
        self.request_frame_rate(DEEP_IDLE_FRAME_RATE);
    }

    fn request_frame_rate(&self, frame_rate: f32) {
        let previous = self
            .requested_frame_rate
            .swap(frame_rate as u32, Ordering::AcqRel);
        if previous == frame_rate as u32 {
            return;
        }
        log::info!("refresh-rate request -> {:.0} Hz", frame_rate);
        let native_window = self.state.lock().native_window.clone();
        log::info!(
            "interaction pacing: native window {}",
            if native_window.is_some() { "attached" } else { "detached" }
        );
        if let Some(native_window) = native_window {
            set_native_frame_rate(&native_window, frame_rate);
        }
    }
    ...
    /// GPUI uses logical pixels while Android WindowInsets uses physical pixels.
    pub fn window_insets(&self) -> gpui::WindowInsets {
        let state = self.state.lock();
        let edges = |insets: SafeAreaInsets| {
            let insets = insets.to_logical(state.scale_factor);
            gpui::Edges {
                top: gpui::px(insets.top),
                right: gpui::px(insets.right),
                bottom: gpui::px(insets.bottom),
                left: gpui::px(insets.left),
            }
        };
        gpui::WindowInsets {
            safe_area: edges(state.safe_area_insets),
            ime: edges(state.ime_insets),
        }
    }

    /// Called on the native UI thread after Java has supplied distinct system
    /// bar/cutout and keyboard insets (including animation progress).
    pub fn update_system_insets(&self, safe_area: SafeAreaInsets, ime: SafeAreaInsets) {
        {
            let mut state = self.state.lock();
            state.has_system_insets = true;
            if state.safe_area_insets == safe_area && state.ime_insets == ime {
                return;
            }
            state.safe_area_insets = safe_area;
            state.ime_insets = ime;
        }
        self.notify_insets_changed();
    }

    fn notify_insets_changed(&self) {
        let callback = self.state.lock().insets_callback.take();
        if let Some(mut callback) = callback {
            callback(self.window_insets());
            let mut state = self.state.lock();
            if state.insets_callback.is_none() {
                state.insets_callback = Some(callback);
            }
        }
    }
    ...
    pub fn update_safe_area_from_content_rect(
        ...
        let mut state = self.state.lock();
        // NativeActivity's content rect cannot separate the IME from bars.
        // Use it only until the host supplies Android's typed WindowInsets.
        if state.has_system_insets {
            return;
        }
        let insets = SafeAreaInsets {
            top: content_top.max(0) as f32,
            bottom: (state.height - content_bottom).max(0) as f32,
            left: content_left.max(0) as f32,
...
impl PlatformWindow for AndroidPlatformWindow {
    fn insets(&self) -> gpui::WindowInsets {
        self.window.window_insets()
    }

    fn on_insets_changed(&self, callback: Box<dyn FnMut(gpui::WindowInsets)>) {
        // Like the existing resize callback, this is registered and invoked
        // exclusively on the native GPUI UI thread; Java only queues values.
        let callback: Box<dyn FnMut(gpui::WindowInsets) + Send> =
            unsafe { std::mem::transmute(callback) };
        self.window.state.lock().insets_callback = Some(callback);
    }
    ...
    fn frame_waker(&self) -> Option<Rc<dyn Fn()>> {
        // GPUI invalidations may happen on the Hermes worker while the native
        // loop is in deep-idle wait. Bridge them to android-activity's looper
        // waker so a dirty surface is rendered immediately.
        let app = crate::android::jni::android_app()?;
        Some(Rc::new(move || app.create_waker().wake()))
    }
    ...
    fn on_request_frame(&self, callback: Box<dyn FnMut(RequestFrameOptions)>) {
        ...
        let android_window = Arc::clone(&self.window);
        ...
        self.window.on_request_frame(move || {
                    ...
                    if let Some(delta) = ms.scroller.step() {
                        ...
                        if let Some(mut guard) = input_cb.try_lock() {
                            ...
                            if fling_ended {
                                ...
                                // `step()` produced the final non-zero
                                // displacement and deactivated the scroller;
                                // there will be no next callback to run the
                                // `None` branch, so restore the balanced rate
                                // here instead of leaving 90 Hz latched.
                                android_window.set_interactive(false);
                            }
                        }
                    } else {
                        ...
                        // Restore the balanced refresh rate once momentum has
                        // fully settled; high refresh is only for interaction.
                        android_window.set_interactive(false);
                    }
                }
            }
            ...
            // GPUI may decide that the scroll-wheel mutation did not leave a
            // dirty view, but native momentum still needs another tick. Keep
            // the frame source armed until `step()` returns `None`; that
            // final callback restores the balanced refresh rate.
            if momentum.lock().scroller.is_active() {
                android_window.schedule_frame();
            }
        });
    }

    fn schedule_frame(&self) {
        self.window.schedule_frame();
    }
                        ...
                        match *ts {
                            TouchState::Pending { start_x, start_y } => {
                                ...
                                if distance > SCROLL_SLOP {
                                    android_window.set_interactive(true);
                                    android_window.schedule_frame();
                    ...
                    1 | 3 => {
                        ...
                        match *ts {
                            ...
                            TouchState::Scrolling { prev_x, prev_y } => {
                                ...
                                let fling_active = ms.scroller.is_active();
                                if fling_active {
                                    android_window.schedule_frame();
                                }
                                ...
                                // A slow drag can fall below MIN_VELOCITY, so
                                // no momentum frame will be generated. Do not
                                // leave the display pinned at 90 Hz in that
                                // path; return to the balanced rate now.
                                if !fling_active {
                                    android_window.set_interactive(false);
                                }
