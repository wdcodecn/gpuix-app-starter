    ...
    <Card.Root>
      ...
      <Card.Content>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          <Badge tone={started ? 'success' : 'neutral'}>{started ? '已响应触摸' : '等待操作'}</Badge>
          <Button variant="primary" testId="starter-action" onClick={() => setStarted((value) => !value)}>{started ? '再次点击' : '开始构建'}</Button>
        </div>
      </Card.Content>
    </Card.Root>
  </div>
